...
const KIND_LABEL: Record<ClubPaymentKind, string> = { COURT_FEE: '월 임대료(코트비)', DEV_FUND: '발전기금' }
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex gap-1 border-b border-(--border-color)">
          {(['COURT_FEE', 'DEV_FUND'] as const).map((k) => (
      ...
      <div className="glass-card rounded-xl overflow-x-auto">
        <table className="w-full text-sm min-w-[1100px]">
          ...
          <tbody>
            ...
            {sorted.length === 0 && <tr><td colSpan={15} className="px-3 py-8 text-center text-(--text-muted)">집계할 거래가 없습니다. 원장에서 거래에 클럽을 연결해주세요.</td></tr>}
          </tbody>
          <tfoot>
      ...
      <p className="text-sm text-(--text-muted)">"미납"은 코트 시간대가 등록된 클럽이 이번 달까지 해당 월 거래가 없을 때 표시됩니다. 두 달치를 한 번에 낸 경우 해당 월에 합산되어 나타납니다.</p>
    </div>
  )
}
