...
const WIZARD_STEP_LABELS = ['레슨 회수', '레슨 시간', '요일', '시간', '요금'] as const
...
export function CreateSlotModal({
  ...
  const canGoNext = (): boolean => {
    ...
    if (step === 4) return times.slice(0, frequency).every(Boolean)
    return true
  }
  ...
  const handleNext = () => {
    ...
    if (nextStep === 5 && feeInput === '') setFeeInput(getDefaultFee())
    setStep(nextStep)
  }
  ...
  const handleSubmit = async () => {
    ...
    const slotInput = {
      ...
      sessions: [] as SlotSession[],
      fee_amount: feeAmount,
      // 날짜 미정 슬롯: 요일 + 시간 정보만 전달
