...
describe('encryptProfile', () => {
  ...
  it('gender는 암호화 대상이 아니므로 그대로 유지된다', () => {
    const result = encryptProfile({ phone: '01012345678', birth_year: '1990', gender: 'M' })
    expect(result.gender).toBe('M') // CHECK 제약 있는 2값 필드라 암호화 제외
  })
...
describe('decryptProfile', () => {
  ...
  it('encrypt → decrypt 왕복 검증', () => {
    const original = { phone: '01012345678', birth_year: '1990', gender: 'M' }
  ...
  it('평문 값이 들어오면 그대로 반환 (마이그레이션 호환)', () => {
    const plain = { phone: '01012345678', birth_year: '1990', gender: 'F' }
  ...
  it('혼재 데이터 처리 (일부 암호화, 일부 평문)', () => {
    ...
    const mixed = { phone: encrypted.phone, birth_year: '1990', gender: 'M' }
