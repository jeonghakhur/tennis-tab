...
// ── 레슨 예약 확정 알림 (슬롯 기반 — 고객 수신) ──────────────────────────

export interface LessonBookingConfirmAlimtalkParams {
  customerPhone: string
  customerName: string
  coachName: string
  /** 계좌 정보 (예: 국민은행 123-456-789 홍길동) */
  bankInfo: string
  /** 레슨 일정 요약 (예: 월 10:00~10:30, 수 10:00~10:30) */
  schedule: string
}

/**
 * 레슨 예약 확정 알림톡 (슬롯 기반, 고객 수신)
 *
 * 알림톡 템플릿:
 * ───────────────────────────────────────────
 * #{고객명}님, 레슨 예약이 확정되었습니다.
 * #{계좌정보}로 입금 부탁드립니다.
 *
 * ■ 레슨 정보
 * - 강사: #{강사명}
 ...
 * ───────────────────────────────────────────
 */
export async function sendLessonBookingConfirmAlimtalk(
  params: LessonBookingConfirmAlimtalkParams,
): Promise<SendResult> {
  const service    = getService()
  const pfId       = process.env.SOLAPI_PFID
  const templateId = process.env.SOLAPI_TEMPLATE_LESSON_BOOKING_CONFIRM
  const sender     = process.env.SOLAPI_SENDER_NUMBER

  if (!service || !pfId || !templateId || !sender) {
    if (process.env.NODE_ENV === 'development') {
      console.info('[Alimtalk DEV] 레슨 예약 확정 알림톡:', params)
      return { success: true, messageId: 'DEV_MOCK' }
    }
    return { success: false, error: '솔라피 환경변수가 설정되지 않았습니다.' }
  }
...
// ── 레슨 예약 취소/거절 알림 (슬롯 기반 — 고객 수신) ─────────────────────

export interface LessonBookingCancelAlimtalkParams {
  customerPhone: string
  customerName: string
  coachName: string
  /** 취소 사유 (없으면 '사유 없음') */
...
/**
 * 레슨 예약 취소/거절 알림톡 (슬롯 기반, 고객 수신)
 *
 * 알림톡 템플릿:
 * ───────────────────────────────────────────
 * #{고객명}님, 레슨 예약이 취소되었습니다.
 *
 * - 강사: #{강사명}
 * - 사유: #{사유}
 *
 * 문의사항은 관리자에게 연락 부탁드립니다.
 * ───────────────────────────────────────────
 */
export async function sendLessonBookingCancelAlimtalk(
  params: LessonBookingCancelAlimtalkParams,
): Promise<SendResult> {
  const service    = getService()
  const pfId       = process.env.SOLAPI_PFID
  const templateId = process.env.SOLAPI_TEMPLATE_LESSON_BOOKING_CANCEL
  const sender     = process.env.SOLAPI_SENDER_NUMBER

  if (!service || !pfId || !templateId || !sender) {
    if (process.env.NODE_ENV === 'development') {
      console.info('[Alimtalk DEV] 레슨 예약 취소 알림톡:', params)
      return { success: true, messageId: 'DEV_MOCK' }
    }
    return { success: false, error: '솔라피 환경변수가 설정되지 않았습니다.' }
  }
