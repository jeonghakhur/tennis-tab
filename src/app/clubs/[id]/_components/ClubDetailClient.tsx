...
export default function ClubDetailClient({ clubId: id }: Props) {
  ...
  const cachedMembership = !authLoading
    ? membershipCache.get(`${id}:${user?.id ?? "guest"}`)
    : undefined;
  ...
  const [membershipChecked, setMembershipChecked] =
    useState(!!cachedMembership);
  const [myMembership, setMyMembership] = useState<PublicMember | null>(
    cachedMembership?.membership ?? null,
  );
  ...
  const [clubAwards, setClubAwards] = useState<
    import("@/lib/supabase/types").Database["public"]["Tables"]["tournament_awards"]["Row"][]
  >([]);
  ...
  useEffect(() => {
    ...
    Promise.all([getClubPublicMembers(id), getClubMemberCount(id)]).then(
      ([membersResult, count]) => {
  ...
  const handleInvitationResponse = async (accept: boolean) => {
    ...
    if (accept) {
      setToast({
        isOpen: true,
        message: `${club?.name ?? "클럽"}에 가입되었습니다!`,
        type: "success",
      });
          ...
          {!isMember && !isSystemAdmin ? (
            <div className="glass-card rounded-xl p-6">
              ...
              {pendingInvitation ? (
                ...
                <div
                  className="mt-6 pt-4 border-t text-center space-y-3"
                  style={{ borderColor: "var(--border-color)" }}
                >
            ...
            <>
              ...
              {/* 모임 탭 */}
              <div
                className={`mt-4${activeTab !== "sessions" ? " hidden" : ""}`}
              >
              ...
              {/* 순위 탭 */}
              <div
                className={`mt-4${activeTab !== "rankings" ? " hidden" : ""}`}
              >
