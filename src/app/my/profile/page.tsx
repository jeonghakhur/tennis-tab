...
export default function MyProfilePage() {
  ...
  const [activeTab, setActiveTab] = useState<'profile' | 'tournaments' | 'matches'>('profile')
      ...
      <main className="min-h-screen pt-20" style={{ backgroundColor: 'var(--bg-primary)' }}>
        <div className="max-w-6xl mx-auto px-6 py-12 text-center">
          <div className="animate-pulse">
            ...
            <div className="h-4 w-64 mx-auto" style={{ backgroundColor: 'var(--bg-card)' }} />
          </div>
        </div>
      </main>
    )
      ...
      <main className="min-h-screen pt-20" style={{ backgroundColor: 'var(--bg-primary)' }}>
          ...
          <div className="flex items-start gap-6">
            ...
            <div className="flex-1">
              ...
              <Link
                ...
                className="inline-block mt-4 px-4 py-2 text-sm rounded-lg transition-all duration-300 hover:scale-105"
                style={{ backgroundColor: 'var(--bg-card-hover)', color: 'var(--text-secondary)' }}
              >
                프로필 수정
              </Link>
        ...
        <div className="flex gap-2 mb-6 border-b" style={{ borderColor: 'var(--border-color)' }}>
          <button
            ...
            className={`px-6 py-3 font-display tracking-wider transition-all duration-300 ${
              activeTab === 'profile' ? 'border-b-2' : ''
            }`}
            style={{
              borderColor: activeTab === 'profile' ? 'var(--accent-color)' : 'transparent',
          ...
          <button
            ...
            style={{
              borderColor: activeTab === 'tournaments' ? 'var(--accent-color)' : 'transparent',
              color: activeTab === 'tournaments' ? 'var(--accent-color)' : 'var(--text-muted)',
            }}
          >
          ...
          <button
            ...
            style={{
              borderColor: activeTab === 'matches' ? 'var(--accent-color)' : 'transparent',
              color: activeTab === 'matches' ? 'var(--accent-color)' : 'var(--text-muted)',
            }}
          >
        ...
        {activeTab === 'profile' && (
          <div className="space-y-4">
            <div className="glass-card p-6">
              <h3 className="text-xl font-display mb-4" style={{ color: 'var(--text-primary)' }}>
                기본 정보
              </h3>
              <div className="space-y-3">
                ...
                <div className="flex justify-between py-2 border-b" style={{ borderColor: 'var(--border-color)' }}>
                  ...
                  <span style={{ color: 'var(--text-primary)' }}>{profile.phone || '미등록'}</span>
                </div>
                <div className="flex justify-between py-2 border-b" style={{ borderColor: 'var(--border-color)' }}>
                  <span style={{ color: 'var(--text-muted)' }}>실력 수준</span>
                  <span style={{ color: 'var(--text-primary)' }}>
                    {profile.skill_level ? skillLevelLabels[profile.skill_level] : '미등록'}
                  </span>
                </div>
                <div className="flex justify-between py-2 border-b" style={{ borderColor: 'var(--border-color)' }}>
                  <span style={{ color: 'var(--text-muted)' }}>주 사용 손</span>
                  <span style={{ color: 'var(--text-primary)' }}>
                    {profile.dominant_hand === 'RIGHT' ? '오른손' :
                     profile.dominant_hand === 'LEFT' ? '왼손' :
                     profile.dominant_hand === 'BOTH' ? '양손' : '미등록'}
                  </span>
                </div>
                <div className="flex justify-between py-2">
                  <span style={{ color: 'var(--text-muted)' }}>소속 클럽</span>
                  <span style={{ color: 'var(--text-primary)' }}>{profile.club || '미등록'}</span>
                </div>
              </div>
            </div>
            ...
            <div className="glass-card p-6">
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="text-center py-4 rounded-lg" style={{ backgroundColor: 'var(--bg-card-hover)' }}>
                  <div className="text-2xl font-display mb-1" style={{ color: 'var(--accent-color)' }}>
                    0
                  </div>
                  <div className="text-sm" style={{ color: 'var(--text-muted)' }}>
                    팔로워
          ...
          <div className="space-y-4">
            {tournaments.length === 0 ? (
              <div className="glass-card p-12 text-center">
                ...
                <Link
                  href="/tournaments"
                  className="inline-block px-6 py-2 rounded-lg font-display tracking-wider transition-all duration-300 hover:scale-105"
                  style={{ backgroundColor: 'var(--accent-color)', color: 'var(--bg-primary)' }}
                >
                  대회 찾아보기
                </Link>
                ...
                <div key={entry.id} className="glass-card p-6">
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <div className="flex flex-col items-end gap-2">
                      <span
                        className={`px-3 py-1 text-xs rounded-full font-display tracking-wider ${
                          entry.status === 'APPROVED' ? 'badge-open' :
                          entry.status === 'PENDING' ? 'badge-progress' :
                          'badge-closed'
                        }`}
                      >
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-sm" style={{ color: 'var(--text-muted)' }}>
                      신청일: {new Date(entry.created_at).toLocaleDateString('ko-KR')}
                    </span>
                    <Link
