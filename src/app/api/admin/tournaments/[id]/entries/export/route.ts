...
export async function GET(request: Request, context: RouteContext) {
  ...
  // 단체전: 팀 선수 목록 — 신청자 본인 참가(applicant_participates !== false) 시 신청자를 맨 앞에 포함
  // (대진표·명예의 전당과 동일한 기준)
  type TeamMemberItem = { name: string; rating: number | null; club?: string | null }
  const getTeamPlayers = (entry: (typeof entries)[number]): TeamMemberItem[] => {
    const members = (entry.team_members as TeamMemberItem[] | null) ?? []
    if (entry.applicant_participates === false) return members
    return [{ name: entry.player_name, rating: entry.player_rating ?? null }, ...members]
  }

  // 단체전: 최대 선수 수 계산 (컬럼 순서 고정용)
  const maxTeamMembers = isTeamMatch
    ? Math.max(0, ...entries.map((e) => getTeamPlayers(e).length))
    : 0
  ...
  const rows: ExcelRow[] = entries.map((entry, index) => {
    ...
    // 단체전: 선수 이름 + 점수 (신청자 참가 시 팀원1 = 신청자, 최대 선수 수만큼 빈 슬롯 포함 → 컬럼 순서 고정)
    if (isTeamMatch) {
      const players = getTeamPlayers(entry)
      const totalRating = players.reduce((sum, m) => sum + (m?.rating ?? 0), 0)
      row['팀 총점'] = totalRating || '-'
      for (let i = 0; i < maxTeamMembers; i++) {
        const player = players[i]
        row[`팀원${i + 1}`] = player?.name || '-'
        row[`팀원${i + 1} 점수`] = player?.rating ?? '-'
      }
    }
